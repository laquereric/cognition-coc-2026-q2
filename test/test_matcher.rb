# frozen_string_literal: true

require "minitest/autorun"
require "cognition-coc-2026-q2"

class MatcherTest < Minitest::Test
  def test_raises_error_without_a_trigger
    assert_raises ArgumentError do
      _ = CognitionCoc2026Q2::Matcher.new(action: "foo")
    end
  end

  def test_raises_error_without_an_action
    assert_raises ArgumentError do
      _ = CognitionCoc2026Q2::Matcher.new(trigger: "foo")
    end
  end

  def test_matches_string
    msg = CognitionCoc2026Q2::Message.new("help")
    matcher = CognitionCoc2026Q2::Matcher.new("help", {}, &proc {})
    assert matcher.matches?(msg)
  end

  def test_string_fails_with_invalid_message
    msg = CognitionCoc2026Q2::Message.new("Help")
    matcher = CognitionCoc2026Q2::Matcher.new("help", {}, &proc {})
    refute matcher.matches?(msg)
  end

  def test_matches_regexp
    msg = CognitionCoc2026Q2::Message.new("ping")
    matcher = CognitionCoc2026Q2::Matcher.new(/ping/, {}, &proc {})
    assert matcher.matches?(msg)
  end

  def test_regexp_fails_with_invalid_message
    msg = CognitionCoc2026Q2::Message.new("pink")
    matcher = CognitionCoc2026Q2::Matcher.new(/ping/, {}, &proc {})
    refute matcher.matches?(msg)
  end

  def test_sets_response_on_attemp_if_matches
    msg = CognitionCoc2026Q2::Message.new("ping")
    matcher = CognitionCoc2026Q2::Matcher.new(/ping/, {}, &proc { "PONG" })
    matcher.attempt(msg)
    assert_equal "PONG", matcher.response
  end

  def test_returns_false_on_attemp_if_no_match
    msg = CognitionCoc2026Q2::Message.new("pink")
    matcher = CognitionCoc2026Q2::Matcher.new(/ping/, {}, &proc { "PONG" })
    refute matcher.attempt(msg)
  end

  def test_sets_match_data
    msg = CognitionCoc2026Q2::Message.new("hello john")
    matcher = CognitionCoc2026Q2::Matcher.new(/hello\s*(?<name>.*)/, {}, &proc { "PONG" })
    matcher.matches?(msg)
    assert_equal "john", matcher.match_data[:name]
  end

  def test_captures_response
    msg = CognitionCoc2026Q2::Message.new("hello john")
    matcher = CognitionCoc2026Q2::Matcher.new(/hello\s*(?<name>.*)/, {}, &proc(&method(:dummy_method)))
    matcher.attempt(msg)
    assert_equal "Hello john", matcher.response
  end

  def test_only_sets_help_when_help_provided
    matcher_without_help = CognitionCoc2026Q2::Matcher.new(/hello\s*(?<name>.*)/, {}, &proc(&method(:dummy_method)))
    assert_equal [], matcher_without_help.help
  end

  def test_sets_help
    matcher_with_help = CognitionCoc2026Q2::Matcher.new(/hello\s*(?<name>.*)/, { help: { "hello" => "says hello" } }, &proc(&method(:dummy_method)))
    assert_equal ["hello - says hello"], matcher_with_help.help
  end

  def test_raises_exception_when_command_fails
    msg = CognitionCoc2026Q2::Message.new("boom")
    matcher = CognitionCoc2026Q2::Matcher.new(/boom/, {}, &proc(&method(:blow_up)))
    matcher.attempt(msg)
    assert matcher.response.include? "ZeroDivisionError"
  end

  def test_empty_response_is_not_a_failure
    msg = CognitionCoc2026Q2::Message.new("empty")
    matcher = CognitionCoc2026Q2::Matcher.new(/empty/, {}, &proc {})
    matcher.attempt(msg)
    assert_equal "", matcher.response
  end
end

def dummy_method(_, match_data)
  "Hello #{match_data['name']}"
end

def blow_up(msg, match_data)
  raise ZeroDivisionError, "divided by 0"
end
